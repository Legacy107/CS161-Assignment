...
void draw_board(int width, int height, const std::vector<std::vector<int>> &board, const std::vector<std::vector<int>> &mask, std::vector<std::vector<int>> &prev_mask, std::pair<int, int> cursor)
{
    ...
    for (int row = 0; row < height; row++)
    {
        ...
        for (int column = 0; column < width; column++)
        {
            if (mask[row][column] != prev_mask[row][column] || manhattan_distance(column, row, cursor.first, cursor.second) <= 1)
            {
                auto background = (cursor.first == column && cursor.second == row) ? INVERSE : 0;
